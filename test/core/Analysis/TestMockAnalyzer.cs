...
namespace Lucene.Net.Analysis
{
	...
	public class TestMockAnalyzer : BaseTokenStreamTestCase
	{
      ...
      public virtual void TestTwoChars()
	  {
		...
		AssertTokenStreamContents(a.TokenStream("bogus", new StreamReader("fooba")), new string[] {"fo", "ob"}, new int[] {0, 2}, new int[] {2, 4}, new int[] {1, 1}, new int?(5)
	   );
        CheckRandomData(new Random(), a, 100);
	  }
      ...
      public virtual void TestThreeChars()
	  {
		...
		// make sure when last term is a "partial" match that End() is correct
        AssertTokenStreamContents(a.TokenStream("bogus", new StreamReader("fooba")), new string[] { "foo" }, new int[] { 0 }, new int[] { 3 }, new int[] { 1 }, new int?(5)
	   );
        CheckRandomData(new Random(), a, 100);
	  }
	  ...
	  private class AnalyzerWrapperAnonymousInnerClassHelper : AnalyzerWrapper
	  {
		  ...
		  public AnalyzerWrapperAnonymousInnerClassHelper(TestMockAnalyzer outerInstance, ReuseStrategy getReuseStrategy, Analyzer @delegate) : base(getReuseStrategy)
		  {
			  this.OuterInstance = outerInstance;
	  ...
	  private class AnalyzerWrapperAnonymousInnerClassHelper2 : AnalyzerWrapper
	  {
		  ...
		  public AnalyzerWrapperAnonymousInnerClassHelper2(TestMockAnalyzer outerInstance, ReuseStrategy getReuseStrategy, int positionGap, int offsetGap, Analyzer @delegate) : base(getReuseStrategy)
		  {
			  this.OuterInstance = outerInstance;
