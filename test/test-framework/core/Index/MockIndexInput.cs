...
namespace Lucene.Net.Index
{
	...
	public class MockIndexInput : BufferedIndexInput
	{
		...
		public MockIndexInput(sbyte[] bytes) : base("MockIndexInput", BufferedIndexInput.BUFFER_SIZE)
		{
			Buffer = bytes;
			Length_Renamed = bytes.Length;
		}

		protected internal override void ReadInternal(sbyte[] dest, int destOffset, int len)
		{
			int remainder = len;
			int start = Pointer;
			while (remainder != 0)
