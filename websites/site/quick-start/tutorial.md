...
We also specify here that title is a `TextField` which means that we want the field to support full text searches, and we specify domain as a `StringField` which means we what to do exact match searches against that field.
