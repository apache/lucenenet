...
namespace Lucene.Net.Index
{
	...
	public class RandomIndexWriter : IDisposable
	{
	  ...
	  public RandomIndexWriter(Random r, Directory dir) : this(r, dir, LuceneTestCase.NewIndexWriterConfig(r, LuceneTestCase.TEST_VERSION_CURRENT, new MockAnalyzer(r)))
	  {
	  }
	  ...
	  public virtual void addDocument<T>(IEnumerable<T> doc) where T : IndexableField
	  {
		AddDocument(doc, w.Analyzer);
	  }

	  public virtual void addDocument<T>(IEnumerable<T> doc, Analyzer a) where T : IndexableField
	  {
		if (r.Next(5) == 3)
		{
	  ...
	  private class IterableAnonymousInnerClassHelper : IEnumerable<IEnumerable<T>>
	  {
		  private readonly RandomIndexWriter OuterInstance;

		  private IEnumerable<T> Doc;

		  public IterableAnonymousInnerClassHelper(RandomIndexWriter outerInstance, IEnumerable<T> doc)
		  {
		  ...
		  private class IteratorAnonymousInnerClassHelper : IEnumerator<IEnumerable<T>>
		  {
			  ...
			  public virtual IEnumerable<T> Next()
			  {
				if (done)
				{
	  ...
	  private class IterableAnonymousInnerClassHelper2 : IEnumerable<IEnumerable<T>>
	  {
		  private readonly RandomIndexWriter OuterInstance;

		  private IEnumerable<T> Doc;

		  public IterableAnonymousInnerClassHelper2(RandomIndexWriter outerInstance, IEnumerable<T> doc)
		  {
			  this.OuterInstance = outerInstance;
			  this.Doc = doc;
		  }
		  ...
		  private class IteratorAnonymousInnerClassHelper2 : IEnumerator<IEnumerable<T>>
		  {
