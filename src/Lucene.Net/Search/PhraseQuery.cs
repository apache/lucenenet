...
namespace Lucene.Net.Search
{
    ...
    public class PhraseQuery : Query, IEnumerable<Term> // LUCENENET specific - implemented IEnumerable<Term>, which allows for use of collection initializer. See: https://stackoverflow.com/a/9195144
    {
        ...
        private class PhraseWeight : Weight
        {
            ...
            {
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(outerInstance.terms.Count > 0)) Debugging.ThrowAssert();
                ...
                for (int i = 0; i < outerInstance.terms.Count; i++)
                {
                    ...
                    {
                        if (Debugging.AssertsEnabled && Debugging.ShouldAssert(TermNotInReader(reader, t))) Debugging.ThrowAssert("no termstate found but term exists in reader");
                        return null;
                    }
                    ...
                    {
                        if (Debugging.AssertsEnabled && Debugging.ShouldAssert(te.SeekExact(t.Bytes))) Debugging.ThrowAssert("termstate found but no term exists in reader");
