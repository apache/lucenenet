...
namespace Lucene.Net.Search
{
        ...
        where T : struct, IComparable<T> // best equiv constraint for java's number class
    {
        ...
        private sealed class NumericRangeTermsEnum : FilteredTermsEnum
        {
                    ...
                    case NumericType.DOUBLE:
                        {
                            ...
                            {
                                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(this.outerInstance.dataType == NumericType.DOUBLE)) Debugging.ThrowAssert();
                    ...
                    case NumericType.SINGLE:
                        {
                            ...
                            {
                                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(this.outerInstance.dataType == NumericType.SINGLE)) Debugging.ThrowAssert();
            ...
            {
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(rangeBounds.Count % 2 == 0)) Debugging.ThrowAssert();

                currentLowerBound = rangeBounds.Dequeue();
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(currentUpperBound == null || termComp.Compare(currentUpperBound, currentLowerBound) <= 0)) Debugging.ThrowAssert("The current upper bound must be <= the new lower bound");
            ...
            protected override sealed BytesRef NextSeekTerm(BytesRef term)
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(rangeBounds.Count == 0)) Debugging.ThrowAssert();
