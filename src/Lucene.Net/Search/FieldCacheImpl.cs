...
namespace Lucene.Net.Search
{
    ...
    internal class FieldCacheImpl : IFieldCache
    {
        ...
        private class ReaderClosedListenerAnonymousInnerClassHelper : IndexReader.IReaderClosedListener
        {
            ...
            {
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(owner is AtomicReader)) Debugging.ThrowAssert();
            ...
            public virtual void DoUninvert(AtomicReader reader, string field, bool setDocsWithField)
            {
                ...
                if (terms != null)
                {
                    if (setDocsWithField)
                    {
                        ...
                        if (Debugging.AssertsEnabled && Debugging.ShouldAssert(termsDocCount <= maxDoc)) Debugging.ThrowAssert();
...
#pragma warning restore IDE0038 // Use pattern matching
            {
                ...
                if (numSet >= maxDoc)
                {
                    ...
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(numSet == maxDoc)) Debugging.ThrowAssert();
...
#pragma warning restore 612, 618
        ...
        internal class Int32sFromArray : FieldCache.Int32s
        {
            ...
            {
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(values.BitsPerValue <= 32)) Debugging.ThrowAssert();
                this.values = values;
