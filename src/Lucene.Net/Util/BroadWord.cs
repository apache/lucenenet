...
namespace Lucene.Net.Util
{
    ...
    public static class BroadWord // LUCENENET specific - made static
    {
        ...
        public static int Select(long x, int r)
        {
            ...
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(0L <= 1)) Debugging.ThrowAssert(l.ToString(CultureInfo.InvariantCulture));
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(r >= 1)) Debugging.ThrowAssert();
            int s = -1;
            while ((x != 0L) && (r > 0))
            {
