...
namespace Lucene.Net.Util.Packed
{
    ...
    public class Packed64 : PackedInt32s.MutableImpl
    {
            ...
            : base(valueCount, bitsPerValue)
        {
            ...
            Debugging.Assert(a == b, "a: " + a, ", b: " + b);*/

            maskRight = (long)((ulong)(~0L << (BLOCK_SIZE - bitsPerValue)) >> (BLOCK_SIZE - bitsPerValue));    //mod

            //Debugging.Assert((long)((ulong)(~0L << (BLOCK_SIZE - bitsPerValue)) >> (BLOCK_SIZE - bitsPerValue)) == (uint)(~0L << (BLOCK_SIZE - bitsPerValue)) >> (BLOCK_SIZE - bitsPerValue));
        ...
        public override int Set(int index, long[] arr, int off, int len)
        {
            ...
            // bulk set
            Debugging.Assert(() => index % encoder.Int64ValueCount == 0);
            int blockIndex = (int)((ulong)((long)index * m_bitsPerValue) >> BLOCK_BITS);
            Debugging.Assert(() => (((long)index * m_bitsPerValue) & MOD_MASK) == 0);
