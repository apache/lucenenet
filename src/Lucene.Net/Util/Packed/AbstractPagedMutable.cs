...
namespace Lucene.Net.Util.Packed
{
    ...
    public abstract class AbstractPagedMutable<T> : Int64Values where T : AbstractPagedMutable<T> // LUCENENET NOTE: made public rather than internal because has public subclasses
    {
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(index >= 0 && index < size)) Debugging.ThrowAssert();
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(minSize >= 0)) Debugging.ThrowAssert();
            if (minSize <= Count)
            {
                T result = (T)this;
                return result;
