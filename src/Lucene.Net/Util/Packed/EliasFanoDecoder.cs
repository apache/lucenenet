...
namespace Lucene.Net.Util.Packed
{
    ...
    public class EliasFanoDecoder
    {
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(((efIndex >= 0) && (efIndex < numEncoded)))) Debugging.ThrowAssert("efIndex {0}", efIndex.ToString(CultureInfo.InvariantCulture));
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(index > efIndex)) Debugging.ThrowAssert();
            if (index >= numEncoded)
            {
                efIndex = numEncoded;
                return false;
        ...
        public virtual long AdvanceToValue(long target)
        {
            ...
            if (indexEntryIndex >= 0) // not before first index entry
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(indexHighValue <= highTarget)) Debugging.ThrowAssert();
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(efIndex < numEncoded)) Debugging.ThrowAssert(); // there is a high value to be found.
            }
            ...
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert((rank <= (sizeof(long) * 8)))) Debugging.ThrowAssert("rank {0}", rank);
            if (rank >= 1)
            {
