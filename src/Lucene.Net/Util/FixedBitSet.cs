...
namespace Lucene.Net.Util
{
    ...
    public sealed class FixedBitSet : DocIdSet, IBits
    {
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(index >= 0 && index < numBits)) Debugging.ThrowAssert($"index={index}, numBits={numBits}");
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(otherNumWords <= numWords)) Debugging.ThrowAssert($"numWords={numWords}, otherNumWords={otherNumWords}");
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(other.numWords <= numWords)) Debugging.ThrowAssert($"numWords={numWords}, other.numWords={other.numWords}");
