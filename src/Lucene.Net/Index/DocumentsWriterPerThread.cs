...
#pragma warning restore 168
        ...
        public DocumentsWriterPerThread(string segmentName, Directory directory, LiveIndexWriterConfig indexWriterConfig, InfoStream infoStream, DocumentsWriterDeleteQueue deleteQueue, FieldInfos.Builder fieldInfos)
        {
            ...
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(numDocsInRAM == 0)) Debugging.ThrowAssert("num docs {0}", numDocsInRAM);
            pendingUpdates.Clear();
            deleteSlice = deleteQueue.NewSlice();

            segmentInfo = new SegmentInfo(directoryOrig, Constants.LUCENE_MAIN_VERSION, segmentName, -1, false, codec, null);
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(numDocsInRAM == 0)) Debugging.ThrowAssert();
        ...
        public virtual int UpdateDocuments(IEnumerable<IEnumerable<IIndexableField>> docs, Analyzer analyzer, Term delTerm)
        {
            ...
            try
            {
                ...
                if (delTerm != null)
                {
                    ...
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(deleteSlice.IsTailItem(delTerm))) Debugging.ThrowAssert("expected the delete term as the tail item");
        ...
        internal virtual FrozenBufferedUpdates PrepareFlush()
        {
            ...
            if (deleteSlice != null)
            {
                ...
                deleteSlice.Apply(pendingUpdates, numDocsInRAM);
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(deleteSlice.IsEmpty)) Debugging.ThrowAssert();
                deleteSlice.Reset();
            }
        ...
        internal virtual FlushedSegment Flush()
        {
            ...
            try
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(segmentInfo != null)) Debugging.ThrowAssert();
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(flushedSegment != null)) Debugging.ThrowAssert();
            ...
            try
            {
                ...
                if (flushedSegment.liveDocs != null)
                {
                    ...
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(delCount > 0)) Debugging.ThrowAssert();
                    if (infoStream.IsEnabled("DWPT"))
                    {
