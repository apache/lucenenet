...
namespace Lucene.Net.Index
{
    ...
    public abstract class LogMergePolicy : MergePolicy
    {
        ...
        {
            if (Debugging.ShouldAssert(maxNumSegments > 0)) Debugging.ThrowAssert();
            if (IsVerbose)
            {
        ...
        public override MergeSpecification FindForcedDeletesMerges(SegmentInfos segmentInfos)
        {
            ...
            IndexWriter w = m_writer.Get();
            if (Debugging.ShouldAssert(w != null)) Debugging.ThrowAssert();
                ...
                while (end <= 1 + upto)
                {
                    ...
                    else if (!anyTooLarge)
                    {
                        ...
                        for (int i = start; i < end; i++)
                        {
                            ...
                            if (Debugging.ShouldAssert(infos.Contains(levels[i].info))) Debugging.ThrowAssert();
                        }
                        if (IsVerbose)
                        {
