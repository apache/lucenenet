    ...
    public sealed class CompositeReaderContext : IndexReaderContext
    {
        ...
        public override IList<AtomicReaderContext> Leaves
        {
            get
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(leaves != null)) Debugging.ThrowAssert();
                return leaves;
            }
        }
        ...
        public sealed class Builder
        {
            ...
            internal IndexReaderContext Build(CompositeReaderContext parent, IndexReader reader, int ord, int docBase)
            {
                ...
                else
                {
                    ...
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(newDocBase == cr.MaxDoc)) Debugging.ThrowAssert();
                    return newParent;
                }
            }
        }
