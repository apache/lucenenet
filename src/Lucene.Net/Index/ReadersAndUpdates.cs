    ...
    internal class ReadersAndUpdates
    {
        ...
        public virtual bool VerifyDocCounts()
        {
            lock (this)
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(Info.Info.DocCount - Info.DelCount - pendingDeleteCount == count)) Debugging.ThrowAssert("info.docCount={0} info.DelCount={1}", Info.Info.DocCount, Info.DelCount + " pendingDeleteCount=" + pendingDeleteCount + " count=" + count);
                return true;
            }
        }
