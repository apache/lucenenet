...
namespace Lucene.Net.Index
{
    ...
    internal class ReadersAndUpdates
    {
        ...
        public virtual void IncRef()
        {
            ...
            if (Debugging.ShouldAssert(rc > 1)) Debugging.ThrowAssert();
        }

        public virtual void DecRef()
        {
            int rc = refCount.DecrementAndGet();
            if (Debugging.ShouldAssert(rc >= 0)) Debugging.ThrowAssert();
        }
        ...
        public virtual bool VerifyDocCounts()
        {
            lock (this)
            {
                ...
                if (Debugging.ShouldAssert(Info.Info.DocCount - Info.DelCount - pendingDeleteCount == count)) Debugging.ThrowAssert("info.docCount={0} info.DelCount={1}", Info.Info.DocCount, Info.DelCount + " pendingDeleteCount=" + pendingDeleteCount + " count=" + count);
                return true;
            }
        }
        ...
        public virtual void Release(SegmentReader sr)
        {
            lock (this)
            {
                if (Debugging.ShouldAssert(Info == sr.SegmentInfo)) Debugging.ThrowAssert();
                sr.DecRef();
            }
        }
        ...
        public virtual SegmentReader GetReadOnlyClone(IOContext context)
        {
            lock (this)
            {
                if (reader == null)
                {
                    ...
                    if (Debugging.ShouldAssert(reader != null)) Debugging.ThrowAssert();
                }
                liveDocsShared = true;
                if (liveDocs != null)
                {
                ...
                {
                    if (Debugging.ShouldAssert(reader.LiveDocs == liveDocs)) Debugging.ThrowAssert();
                    reader.IncRef();
                    return reader;
                }
            }
        ...
        public virtual bool WriteLiveDocs(Directory dir)
        {
            lock (this)
            {
                ...
                // We have new deletes
                if (Debugging.ShouldAssert(liveDocs.Length == Info.Info.DocCount)) Debugging.ThrowAssert();
        ...
        public virtual void WriteFieldUpdates(Directory dir, DocValuesFieldUpdates.Container dvUpdates)
        {
            lock (this)
            {
                ...
                if (Debugging.ShouldAssert(dvUpdates.Any())) Debugging.ThrowAssert();
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            foreach (KeyValuePair<string, NumericDocValuesFieldUpdates> e in dvUpdates.numericDVUpdates)
                            {
                                ...
                                if (Debugging.ShouldAssert(fieldInfo != null)) Debugging.ThrowAssert();
        ...
        private IEnumerable<long?> GetInt64Enumerable(SegmentReader reader, string field, NumericDocValuesFieldUpdates fieldUpdates)
        {
            ...
            for (int curDoc = 0; curDoc < maxDoc; ++curDoc)
            {
                ...
                {   // no update for this document
                    if (Debugging.ShouldAssert(curDoc < updateDoc)) Debugging.ThrowAssert();
