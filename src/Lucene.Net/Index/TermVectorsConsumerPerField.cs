...
namespace Lucene.Net.Index
{
    ...
    internal sealed class TermVectorsConsumerPerField : TermsHashConsumerPerField
    {
        ...
        {
            if (Debugging.ShouldAssert(docState.TestPoint("TermVectorsTermsWriterPerField.finish start"))) Debugging.ThrowAssert();

            int numPostings = termsHashPerField.bytesHash.Count;

            BytesRef flushTerm = termsWriter.flushTerm;

            if (Debugging.ShouldAssert(numPostings >= 0)) Debugging.ThrowAssert();
            ...
            if (Debugging.ShouldAssert(termsWriter.VectorFieldsInOrder(fieldInfo))) Debugging.ThrowAssert();
        ...
        {
            if (Debugging.ShouldAssert(docState.TestPoint("TermVectorsTermsWriterPerField.newTerm start"))) Debugging.ThrowAssert();
            TermVectorsPostingsArray postings = (TermVectorsPostingsArray)termsHashPerField.postingsArray;
        ...
        {
            if (Debugging.ShouldAssert(docState.TestPoint("TermVectorsTermsWriterPerField.addTerm start"))) Debugging.ThrowAssert();
            TermVectorsPostingsArray postings = (TermVectorsPostingsArray)termsHashPerField.postingsArray;
        ...
        internal sealed class TermVectorsPostingsArray : ParallelPostingsArray
        {
            ...
            {
                if (Debugging.ShouldAssert(toArray is TermVectorsPostingsArray)) Debugging.ThrowAssert();
