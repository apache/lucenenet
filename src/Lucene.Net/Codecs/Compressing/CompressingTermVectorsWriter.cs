...
namespace Lucene.Net.Codecs.Compressing
{
    ...
    public sealed class CompressingTermVectorsWriter : TermVectorsWriter
    {
        ...
        {
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(curField.flags != 0);
        ...
        private int[] FlushFieldNums()
        {
            ...
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(numDistinctFields > 0);
        ...
        private void FlushFields(int totalFields, int[] fieldNums)
        {
            ...
            foreach (DocData dd in pendingDocs)
            {
                foreach (FieldData fd in dd.fields)
                {
                    ...
                    if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(fieldNumIndex >= 0);
        ...
        private void FlushFlags(int totalFields, int[] fieldNums)
        {
            ...
            foreach (DocData dd in pendingDocs)
            {
                ...
                foreach (FieldData fd in dd.fields)
                {
                    ...
                    if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(fieldNumOff >= 0);
            ...
            if (nonChangingFlags)
            {
                ...
                {
                    if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(flags >= 0);
                    writer.Add(flags);
                }
                if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(writer.Ord == fieldFlags.Length - 1);
                writer.Finish();
            }
            else
            {
                ...
                if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(writer.Ord == totalFields - 1);
                writer.Finish();
            }
        }
            ...
            foreach (DocData dd in pendingDocs)
            {
                foreach (FieldData fd in dd.fields)
                {
                    if (fd.hasPositions)
                    {
                        ...
                        if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(pos == fd.totalPositions);
                    }
                }
            }
            writer.Finish();
                ...
                else
                {
                    ...
                    for (int i = NextLiveDoc(0, liveDocs, maxDoc); i < maxDoc; )
                    {
                        ...
                        if ((pendingDocs.Count == 0) && (i == 0 || index.GetStartPointer(i - 1) < startPointer)) // start of a chunk
                        {
                            ...
                            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(docBase + chunkDocs <= matchingSegmentReader.MaxDoc);
