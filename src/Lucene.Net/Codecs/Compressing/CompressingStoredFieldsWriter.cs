    ...
    public sealed class CompressingStoredFieldsWriter : StoredFieldsWriter
    {
        ...
        private void Flush()
        {
            ...
            for (int i = numBufferedDocs - 1; i > 0; --i)
            {
                ...
                if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(lengths[i] >= 0);
            }
        ...
        public override void Finish(FieldInfos fis, int numDocs)
        {
            ...
            {
                if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(bufferedDocs.Length == 0);
            }
            if (docBase != numDocs)
            {
                ...
                else
                {
                    ...
                    if (docID < maxDoc)
                    {
                        ...
                        do
                        {
                            ...
                            {
                                if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(docID == it.docBase);
