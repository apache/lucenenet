    ...
    public sealed class CompressingStoredFieldsWriter : StoredFieldsWriter
    {
        ...
        private void Flush()
        {
            ...
            for (int i = numBufferedDocs - 1; i > 0; --i)
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(lengths[i] >= 0)) Debugging.ThrowAssert();
            }
        ...
        public override void Finish(FieldInfos fis, int numDocs)
        {
            ...
            {
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(bufferedDocs.Length == 0)) Debugging.ThrowAssert();
            }
            if (docBase != numDocs)
            {
                ...
                else
                {
                    ...
                    if (docID < maxDoc)
                    {
                        ...
                        do
                        {
                            ...
                            {
                                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(docID == it.docBase)) Debugging.ThrowAssert();
