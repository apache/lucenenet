...
#pragma warning restore 612, 618
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(CodecUtil.HeaderLength(codecNameDat) == vectorsStream.GetFilePointer())) Debugging.ThrowAssert();
        ...
        public override Fields Get(int doc)
        {
            ...
            int[] fieldNums;
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(token != 0)) Debugging.ThrowAssert(); // means no term vectors, cannot happen since we checked for numFields == 0
                ...
                switch (vectorsStream.ReadVInt32())
                {
                    case 0:
                        ...
                        for (int i = 0; i < totalFields; ++i)
                        {
                            ...
                            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(fieldNumOff >= 0 && fieldNumOff < fieldNums.Length)) Debugging.ThrowAssert();
            ...
            for (int i = 0, termIndex = 0; i < totalFields; ++i)
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(i != totalFields - 1 || termIndex == totalTerms)) Debugging.ThrowAssert("{0} {1}", termIndex, totalTerms);
            }
            ...
            if (totalPayloads > 0)
            {
                ...
                for (int i = 0; i < numFields; ++i)
                {
                    ...
                    if ((f & CompressingTermVectorsWriter.PAYLOADS) != 0)
                    {
                        ...
                        if (Debugging.AssertsEnabled && Debugging.ShouldAssert(posIdx == totalFreq)) Debugging.ThrowAssert();
                    }
                    termIndex += termCount;
                }
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(termIndex == totalTerms)) Debugging.ThrowAssert("{0} {1}", termIndex, totalTerms);
            }
            ...
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(Sum(fieldLengths) == docLen)) Debugging.ThrowAssert("{0} != {1}", Sum(fieldLengths), docLen);
        ...
        private class TVFields : Fields
        {
            ...
            public override Terms GetTerms(string field)
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(fieldLen >= 0)) Debugging.ThrowAssert();
        ...
        private class TVTermsEnum : TermsEnum
        {
            ...
            public override bool MoveNext()
            {
                ...
                {
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(ord < numTerms)) Debugging.ThrowAssert();
                    ++ord;
                }
