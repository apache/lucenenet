            ...
            while (true)
            {
                if (current == null)
                {
                    ...
                    else
                    {
                        ...
                        if (Debugging.AssertsEnabled && Debugging.ShouldAssert(currentMap.MaxDoc == subs[upto].Slice.Length)) Debugging.ThrowAssert("readerIndex={0} subs.len={1}", reader, subs.Length + " len1=" + currentMap.MaxDoc + " vs " + subs[upto].Slice.Length);
                    }
                }
