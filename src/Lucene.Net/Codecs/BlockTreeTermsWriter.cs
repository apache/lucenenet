    ...
    public class BlockTreeTermsWriter : FieldsConsumer
    {
        ...
        private class FieldMetaData
        {
            ...
            public FieldMetaData(FieldInfo fieldInfo, BytesRef rootCode, long numTerms, long indexStartFP, long sumTotalTermFreq, long sumDocFreq, int docCount, int longsSize)
            {
                ...
                Debugging.Assert(() => rootCode != null, () => "field=" + fieldInfo.Name + " numTerms=" + numTerms);
        ...
        public override TermsConsumer AddField(FieldInfo field)
        {
            ...
            Debugging.Assert(() => currentField == null || currentField.Name.CompareToOrdinal(field.Name) < 0);
            currentField = field;
        ...
        private sealed class PendingBlock : PendingEntry
        {
            ...
            public void CompileIndex(IList<PendingBlock> floorBlocks, RAMOutputStream scratchBytes)
            {
                ...
                Debugging.Assert(() => bytes.Length > 0);
        ...
        internal class TermsWriter : TermsConsumer
        {
                ...
                else
                {
                    ...
                    foreach (PendingEntry ent in slice)
                    {
                        ...
                        else
                        {
                            ...
                            Debugging.Assert(() => block.Prefix.Length > prefixLength);
                    ...
                    */
                    ...
                    for (int sub = 0; sub < numSubs; sub++)
                    {
                        ...
                        if (pendingCount >= outerInstance.minItemsInBlock)
                        {
                            ...
                            Debugging.Assert(() => outerInstance.minItemsInBlock == 1 || subCount > 1, () => "minItemsInBlock=" + outerInstance.minItemsInBlock + " subCount=" + subCount + " sub=" + sub + " of " + numSubs + " subTermCount=" + subTermCountSums[sub] + " subSubCount=" + subSubCounts[sub] + " depth=" + prefixLength);
                            subCount = 0;
            ...
            private PendingBlock WriteBlock(Int32sRef prevTerm, int prefixLength, int indexPrefixLength, int startBackwards, int length, int futureTermCount, bool isFloor, int floorLeadByte, bool isLastInFloor)
            {
                ...
                Debugging.Assert(() => start >= 0, () => "pending.Count=" + pending.Count + " startBackwards=" + startBackwards + " length=" + length);
                ...
                if (isLeafBlock)
                {
                    ...
                    foreach (PendingEntry ent in slice)
                    {
                        ...
                        {
                            Debugging.Assert(() => state.TotalTermFreq >= state.DocFreq, () => state.TotalTermFreq + " vs " + state.DocFreq);
                ...
                else
                {
                    ...
                    foreach (PendingEntry ent in slice)
                    {
                        if (ent.IsTerm)
                        {
                            ...
                            if (fieldInfo.IndexOptions != IndexOptions.DOCS_ONLY)
                            {
                                Debugging.Assert(() => state.TotalTermFreq >= state.DocFreq);
                                statsWriter.WriteVInt64(state.TotalTermFreq - state.DocFreq);
                            }
                        ...
                        else
                        {
                            ...
                            Debugging.Assert(() => suffix > 0);

                            // For non-leaf block we borrow 1 bit to record
                            // if entry is term or sub-block
                            suffixWriter.WriteVInt32((suffix << 1) | 1);
                            suffixWriter.WriteBytes(block.Prefix.Bytes, prefixLength, suffix);
                            Debugging.Assert(() => block.Fp < startFP);
                    ...
                    Debugging.Assert(() => subIndices.Count != 0);
                }
