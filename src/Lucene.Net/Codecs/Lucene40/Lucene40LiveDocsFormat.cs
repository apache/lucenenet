...
namespace Lucene.Net.Codecs.Lucene40
{
    ...
    public class Lucene40LiveDocsFormat : LiveDocsFormat
    {
        ...
        public override IBits ReadLiveDocs(Directory dir, SegmentCommitInfo info, IOContext context)
        {
            ...
            {
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(liveDocs.Count() == info.Info.DocCount - info.DelCount)) Debugging.ThrowAssert("liveDocs.count()={0} info.docCount={1}", liveDocs.Count(), info.Info.DocCount + " info.getDelCount()=" + info.DelCount);
                Debugging.ThrowAssertIf(liveDocs.Length == info.Info.DocCount);
            }
            return liveDocs;
        }
        ...
        public override void WriteLiveDocs(IMutableBits bits, Directory dir, SegmentCommitInfo info, int newDelCount, IOContext context)
        {
            ...
            if (Debugging.AssertsEnabled)
            {
                Debugging.ThrowAssertIf(liveDocs.Count() == info.Info.DocCount - info.DelCount - newDelCount);
                Debugging.ThrowAssertIf(liveDocs.Length == info.Info.DocCount);
            }
