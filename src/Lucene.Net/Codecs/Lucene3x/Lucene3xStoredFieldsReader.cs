        ...
        public Lucene3xStoredFieldsReader(Directory d, SegmentInfo si, FieldInfos fn, IOContext context)
        {
            ...
            try
            {
                ...
                if (docStoreOffset != -1)
                {
                    ...
                    // docs
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(((int)(indexSize / 8)) >= size + this.docStoreOffset)) Debugging.ThrowAssert("indexSize={0} size={1}", indexSize, size + " docStoreOffset=" + docStoreOffset);
                }
                else
                {
...
#pragma warning restore 168
        ...
        public override sealed void VisitDocument(int n, StoredFieldVisitor visitor)
        {
            ...
            for (int fieldIDX = 0; fieldIDX < numFields; fieldIDX++)
            {
                ...
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(bits <= (FIELD_IS_NUMERIC_MASK | FIELD_IS_BINARY))) Debugging.ThrowAssert("bits={0}", bits.ToString("x"));
