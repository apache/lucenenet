...
namespace Lucene.Net.Spatial
{
    ...
    public abstract class SpatialTestCase : LuceneTestCase
    {
        ...
        {
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(mean > min);
        ...
        private double randomGaussianMeanMax(double mean, double max)
        {
            ...
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(mean <= max && mean >= 0);
            ...
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(pivot >= 0 && pivotMax >= pivot && g >= 0);
            double pivotResult;
            if (g <= 1)
                pivotResult = pivot * g;
            else
