...
namespace Lucene.Net.Spatial
{
    ...
    public class DisjointSpatialFilter : Filter
    {
        ...
        public override DocIdSet GetDocIdSet(AtomicReaderContext context, IBits acceptDocs)
        {
            ...
            else
            {
                ...
                if (docsWithField.Length != maxDoc)
                {
                    throw new InvalidOperationException(
                        string.Format("Bits length should be maxDoc ({0}) but wasn't: {1}", maxDoc, docsWithField));
                }
            ...
            var docIdSet = new ChainedFilter(new Filter[] { intersectsFilter }, ChainedFilter.ANDNOT).GetDocIdSet(context, acceptDocs);
        ...
        {
            var result = field != null ? field.GetHashCode() : 0;
