    ...
    public class ReplicationClient : IDisposable
    {
        ...
        protected virtual IDictionary<string, IList<RevisionFile>> RequiredFiles(IDictionary<string, IList<RevisionFile>> newRevisionFiles)
        {
            ...
            foreach (var e in handlerRevisionFiles)
            {
                ...
                string source = e.Key;
                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(newRevisionFiles.ContainsKey(source))) Debugging.ThrowAssert("source not found in newRevisionFiles: {0}", newRevisionFiles);
        ...
        public virtual void StartUpdateThread(long intervalMillis, string threadName)
        {
            ...
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(updateThread.IsAlive)) Debugging.ThrowAssert("updateThread started but not alive?");
        }
