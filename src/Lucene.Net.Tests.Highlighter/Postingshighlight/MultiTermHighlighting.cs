...
namespace Org.Apache.Lucene.Search.Postingshighlight
{
	...
	internal class MultiTermHighlighting
	{
												...
												if (query is FuzzyQuery)
												{
													...
													if (fq.GetField().Equals(field))
													{
														...
														LevenshteinAutomata builder = new LevenshteinAutomata(suffix, fq.GetTranspositions
															());
														Org.Apache.Lucene.Util.Automaton.Automaton automaton = builder.ToAutomaton(fq.GetMaxEdits
															());
														if (prefixLength > 0)
														{
															Org.Apache.Lucene.Util.Automaton.Automaton prefix = BasicAutomata.MakeString(UnicodeUtil
		...
		{
			public _CharacterRunAutomaton_92(AutomatonQuery aq, Org.Apache.Lucene.Util.Automaton.Automaton
		...
		{
			public _CharacterRunAutomaton_104(PrefixQuery pq, Org.Apache.Lucene.Util.Automaton.Automaton
				 baseArg1) : base(baseArg1)
			{
				this.pq = pq;
			}
		...
		{
			public _CharacterRunAutomaton_128(FuzzyQuery fq, Org.Apache.Lucene.Util.Automaton.Automaton
				 baseArg1) : base(baseArg1)
			{
				this.fq = fq;
			}
		...
		private sealed class _CharacterRunAutomaton_158 : CharacterRunAutomaton
		{
			public _CharacterRunAutomaton_158(CharsRef scratch, CharsRef lowerBound, IComparer
				...
				, TermRangeQuery tq, Org.Apache.Lucene.Util.Automaton.Automaton baseArg1) : base
				(baseArg1)
			{
