...
namespace Lucene.Net.Analysis.Icu.Segmentation
{
    ...
    public class ICUTokenizerFactory : TokenizerFactory, IResourceLoaderAware
    {
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(tailored != null)) Debugging.ThrowAssert("init must be called first!");
            if (tailored.Count == 0)
            {
        ...
        {
            if (Debugging.AssertsEnabled && Debugging.ShouldAssert(config != null)) Debugging.ThrowAssert("inform must be called first!");
