...
namespace Lucene.Net.Analysis.CharFilters
{
    ...
    public class HTMLStripCharFilterTest : BaseTokenStreamTestCase
    {
        ...
        {
            int maxNumElems = 100;
            string randomHtmlishString1 = TestUtil.RandomHtmlishString(Random(), maxNumElems).Replace(">", " ").replaceFirst("^--", "__"); // Don't create a comment (disallow "<!--") and don't include a closing ">"
            string closedAngleBangNonCDATA = "<!" + randomHtmlishString1 + "-[CDATA[&]]>";

            string randomHtmlishString2 = TestUtil.RandomHtmlishString(Random(), maxNumElems).Replace(">", " ").replaceFirst("^--", "__"); // Don't create a comment (disallow "<!--") and don't include a closing ">"
