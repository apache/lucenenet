...
namespace Lucene.Net.Spatial.Prefix
{
    ...
    public abstract class AbstractVisitingPrefixTreeFilter : AbstractPrefixTreeFilter
    {
        ...
        public abstract class VisitorTemplate : BaseTermsEnumTraverser
        {
            ...
            */
            ...
            public virtual DocIdSet GetDocIdSet()
            {
                ...
                while (thisTerm != null)//terminates for other reasons too!
                {
                    ...
                    if (compare > 0)
                    {
                        ...
                        Debugging.Assert(() => !m_context.AtomicReader.GetTerms(m_outerInstance.m_fieldName).GetIterator(null).SeekExact(curVNodeTerm), () => "should be absent");
                    }
                    else
                    {
                        if (compare < 0)
            ...
            private class VNodeCellIterator : IEnumerator<VNode>
            {
                ...
                {
                    //Debugging.Assert(cellIter.Current != null);
        ...
        public class VNode
        {
            ...
            {
                Debugging.Assert(() => cell != null);
                this.cell = cell;
                Debugging.Assert(() => children == null);
            }
        }
