...
//ORIGINAL LINE: final int startTermOrd = termOrd;
						...
						while (termOrd < outerInstance.terms.Length && outerInstance.compare(termOrd, startTerm) <= 0)
						{
						  Debug.Assert(termOrd == startTermOrd || outerInstance.skipOffsets[termOrd] == outerInstance.skipOffsets[termOrd + 1]);
						  termOrd++;
						}
...
//ORIGINAL LINE: final int termLen = termOffsets[1+termOrd] - termOffset;
			  ...
			  if (termOrd >= 0 && !startTerm.Equals(new BytesRef(outerInstance.termBytes, termOffset, termLen)))
			  {
				stateUpto -= skipUpto;
				termOrd--;
			  }
...
//ORIGINAL LINE: final int targetLabel = state.transitionMin;
			  ...
			  if ((outerInstance.termBytes[termOffset + stateUpto] & 0xFF) < targetLabel)
			  {
				...
				while (true)
				{
				  ...
				  int mid = (int)((uint)(low + high) >> 1);
				  int cmp = (outerInstance.termBytes[outerInstance.termOffsets[mid] + stateUpto] & 0xFF) - targetLabel;
				  // if (DEBUG) {
				  ...
				  else
				  {
					...
					// occurrence:
					while (mid > termOrd && (outerInstance.termBytes[outerInstance.termOffsets[mid - 1] + stateUpto] & 0xFF) == targetLabel)
					{
					  mid--;
					}
					termOrd = mid;
			  ...
			  else
			  {
				...
				if (compiledAutomaton.commonSuffixRef != null)
				{
				  ...
				  {
					if (outerInstance.termBytes[offset + suffix] != compiledAutomaton.commonSuffixRef.bytes[suffix])
					{
					  termOrd++;
					  skipUpto = 0;
					  goto nextTermContinue;
	  ...
	  private sealed class HighFreqDocsAndPositionsEnum : DocsAndPositionsEnum
	  {
		...
		public override BytesRef Payload
		{
			get
			{
			  ...
			  {
	//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
	//ORIGINAL LINE: final byte[] payloadBytes = payloads[upto][posUpto/(hasOffsets ? 3:1)];
				sbyte[] payloadBytes = payloads[upto][posUpto / (hasOffsets ? 3:1)];
