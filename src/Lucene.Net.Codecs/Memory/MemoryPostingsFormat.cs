    ...
    public sealed class MemoryPostingsFormat : PostingsFormat
    {
        ...
        private sealed class TermsWriter : TermsConsumer
        {
            ...
            private class PostingsWriter : PostingsConsumer
            {
                ...
                public override void StartDoc(int docID, int termDocFreq)
                {
                    ...
                    if (Debugging.ShouldAssert(docID == 0 || delta > 0)) Debugging.ThrowAssert();
                    lastDocID = docID;
                    docCount++;
                    ...
                    else
                    {
                        ...
                        if (Debugging.ShouldAssert(termDocFreq > 0)) Debugging.ThrowAssert();
                ...
                {
                    if (Debugging.ShouldAssert(payload == null || outerInstance.field.HasPayloads)) Debugging.ThrowAssert();
                ...
                {
                    if (Debugging.ShouldAssert(buffer.GetFilePointer() == 0)) Debugging.ThrowAssert();
                    lastDocID = 0;
                    docCount = 0;
                    lastPayloadLen = 0;
                    lastOffsetLength = -1;
            ...
            {
                if (Debugging.ShouldAssert(postingsWriter.docCount == stats.DocFreq)) Debugging.ThrowAssert();

                if (Debugging.ShouldAssert(buffer2.GetFilePointer() == 0)) Debugging.ThrowAssert();
        ...
        private sealed class FSTDocsAndPositionsEnum : DocsAndPositionsEnum
        {
            ...
            public override int NextPosition()
            {
                ...
                if (Debugging.ShouldAssert(posPending > 0)) Debugging.ThrowAssert();
                posPending--;
                if (!storePayloads)
                {
                    pos += @in.ReadVInt32();
