...
namespace Lucene.Net.QueryParsers.Simple
{
    ...
    public class SimpleQueryParser : QueryBuilder
    {
        ...
        {
            if (Debugging.ShouldAssert((m_flags & Operator.PRECEDENCE_OPERATORS) != 0)) Debugging.ThrowAssert();
        ...
        {
            if (Debugging.ShouldAssert((m_flags & Operator.PHRASE_OPERATOR) != 0)) Debugging.ThrowAssert();
            int start = ++state.Index;
            int copied = 0;
            bool escaped = false;
            bool hasSlop = false;
