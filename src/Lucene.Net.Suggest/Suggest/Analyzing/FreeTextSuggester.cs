        ...
        public virtual IList<LookupResult> DoLookup(string key, IEnumerable<BytesRef> contexts, int num)
        {
            ...
            try
            {
                ...
                while (ts.IncrementToken())
                {
                    ...
                    if (Debugging.ShouldAssert(gramCount <= grams)) Debugging.ThrowAssert();
                ...
                for (int gram = grams - 1; gram >= 0; gram--)
                {
                    ...
                    if (Debugging.ShouldAssert(finalLastToken.Offset == 0)) Debugging.ThrowAssert();
                    ...
                    foreach (Util.Fst.Util.Result<long?> completion in completions)
                    {
                        ...
                        for (int i = token.Length - 1; i >= 0; i--)
                        {
                            if (token.Bytes[token.Offset + i] == separator)
                            {
                                if (Debugging.ShouldAssert(token.Length - i - 1 > 0)) Debugging.ThrowAssert();
                        ...
                        results.Add(result);
                        if (Debugging.ShouldAssert(results.Count == seen.Count)) Debugging.ThrowAssert();
