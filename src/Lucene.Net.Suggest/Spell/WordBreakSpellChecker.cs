...
namespace Lucene.Net.Search.Spell
{
    ...
    public class WordBreakSpellChecker
    {
            ...
            BreakSuggestionSortMethod sortMethod = BreakSuggestionSortMethod.NUM_CHANGES_THEN_MAX_FREQUENCY)
        {
            if (maxSuggestions < 1)
            {
                return Array.Empty<SuggestWord[]>();
            }
            ...
            GenerateBreakUpSuggestions(term, ir, 1, maxSuggestions, useMinSuggestionFrequency, Array.Empty<SuggestWord>(), suggestions, 0, sortMethod);
            ...
            IndexReader ir, SuggestMode suggestMode = SuggestMode.SUGGEST_WHEN_NOT_IN_INDEX)
        {
            if (maxSuggestions < 1)
            {
                return Array.Empty<CombineSuggestion>();
            }
