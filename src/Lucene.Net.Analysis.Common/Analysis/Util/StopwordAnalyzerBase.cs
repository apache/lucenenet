...
namespace Lucene.Net.Analysis.Util
{
	...
	public abstract class StopwordAnalyzerBase : Analyzer
	{
		...
		/// <param name="version">
		///          the Lucene version for cross version compatibility </param>
		protected internal StopwordAnalyzerBase(LuceneVersion version) : this(version, null)
		{
		}
		...
		protected internal static CharArraySet LoadStopwordSet(bool ignoreCase, Type aClass, string resource, string comment)
		{
			...
			try
			{
				...
				resourceStream = assembly.GetManifestResourceStream(aClass, resource);
			...
			{
				IOUtils.Close(resourceStream, reader);
			}
		}
		...
		/// <exception cref="IOException">
		///           if loading the stopwords throws an <seealso cref="IOException"/> </exception>
		protected internal static CharArraySet LoadStopwordSet(Stream stopwords, LuceneVersion matchVersion)
		{
			TextReader reader = null;
			try
			{
