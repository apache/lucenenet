...
namespace Lucene.Net.Codecs.Lucene41
{
	...
	internal sealed class Lucene41SkipReader : MultiLevelSkipListReader
	{
	  ...
	  public Lucene41SkipReader(IndexInput skipStream, int maxSkipLevels, int blockSize, bool hasPos, bool hasOffsets, bool hasPayloads) : base(skipStream, maxSkipLevels, blockSize, 8)
	  {
	  ...
	  public void Init(long skipPointer, long docBasePointer, long posBasePointer, long payBasePointer, int df)
	  {
		...
		Arrays.fill(DocPointer_Renamed, docBasePointer);
		if (PosPointer_Renamed != null)
		{
		  Arrays.fill(PosPointer_Renamed, posBasePointer);
		  if (PayPointer_Renamed != null)
		  {
			Arrays.fill(PayPointer_Renamed, payBasePointer);
		  }
		}
		else
		{
