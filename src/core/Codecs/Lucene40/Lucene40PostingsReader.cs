...
//ORIGINAL LINE: final int newOrd = skipper.skipTo(target);
	  ...
	  private sealed class AllDocsSegmentDocsEnum : SegmentDocsEnumBase
	  {
		...
		internal AllDocsSegmentDocsEnum(Lucene40PostingsReader outerInstance, IndexInput startFreqIn) : base(outerInstance, startFreqIn, null)
		{
			this.OuterInstance = outerInstance;
...
//ORIGINAL LINE: final int loopLimit = limit;
	  ...
	  private sealed class LiveDocsSegmentDocsEnum : SegmentDocsEnumBase
	  {
		...
		internal LiveDocsSegmentDocsEnum(Lucene40PostingsReader outerInstance, IndexInput startFreqIn, Bits liveDocs) : base(outerInstance, startFreqIn, liveDocs)
		{
			this.OuterInstance = outerInstance;
		  Debug.Assert(liveDocs != null);
		}
