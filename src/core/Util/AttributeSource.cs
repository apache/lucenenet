...
namespace Lucene.Net.Util
{
    ...
    public class AttributeSource
    {
        ...
        {
            LinkedList<WeakReference> foundInterfaces = knownImplClasses[clazz];
        ...
        public virtual void AddAttributeImpl(Attribute attr)
        {
            ...
            LinkedList<WeakReference> foundInterfaces = GetAttributeInterfaces(attrType);
        ...
        private class ReflectAsStringReflector : IAttributeReflector
        {
            private readonly StringBuilder buffer;
            private readonly bool prependAttClass;

            public ReflectAsStringReflector(StringBuilder buffer, bool prependAttClass)
            {
                this.buffer = buffer;
        ...
        public string ReflectAsString(bool prependAttClass)
        {
            ...
            ReflectWith(new ReflectAsStringReflector(buffer, prependAttClass));
        ...
        public void CopyTo(AttributeSource target)
        {
            for (State state = GetCurrentState(); state != null; state = state.next)
            {
                ...
                if (targetImpl == null)
                {
                    throw new ArgumentException("This AttributeSource contains AttributeImpl of type " +
                      state.attribute.GetType().Name + " that is not in the target");
                }
