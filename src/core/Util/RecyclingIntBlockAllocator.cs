...
namespace Lucene.Net.Util
{
	...
	public sealed class RecyclingIntBlockAllocator : Allocator
	{
	  ...
	  public RecyclingIntBlockAllocator(int blockSize, int maxBufferedBlocks, Counter bytesUsed) : base(blockSize)
	  {
	  ...
	  public RecyclingIntBlockAllocator(int blockSize, int maxBufferedBlocks) : this(blockSize, maxBufferedBlocks, Counter.NewCounter(false))
	  {
	  }
	  ...
	  /// 
	  /// </summary>
	  public RecyclingIntBlockAllocator() : this(IntBlockPool.INT_BLOCK_SIZE, 64, Counter.NewCounter(false))
	  {
	  }
	...
	//ORIGINAL LINE: final int[] b = freeByteBlocks[--freeBlocks];
...
//ORIGINAL LINE: final int size = freeBlocks + numBlocks;
		...
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final int[][] newBlocks = new int[ArrayUtil.oversize(size, RamUsageEstimator.NUM_BYTES_OBJECT_REF)][];
