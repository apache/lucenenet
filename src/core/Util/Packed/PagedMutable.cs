...
namespace Lucene.Net.Util.Packed
{
	...
	public sealed class PagedMutable : AbstractPagedMutable<PagedMutable>
	{
	  ...
	  public PagedMutable(long size, int pageSize, int bitsPerValue, float acceptableOverheadRatio) : this(size, pageSize, PackedInts.FastestFormatAndBits(pageSize, bitsPerValue, acceptableOverheadRatio))
	  {
		FillPages();
	  }
	  ...
	  internal PagedMutable(long size, int pageSize, int bitsPerValue, PackedInts.Format format) : base(bitsPerValue, size, pageSize)
	  {
		this.Format = format;
	  }
