...
namespace Lucene.Net.Util.Packed
{
	...
	internal sealed class PackedReaderIterator : PackedInts.ReaderIteratorImpl
	{
	  ...
	  internal PackedReaderIterator(PackedInts.Format format, int packedIntsVersion, int valueCount, int bitsPerValue, DataInput @in, int mem) : base(valueCount, bitsPerValue, @in)
	  {
		this.Format = format;
...
//ORIGINAL LINE: final int remaining = valueCount - position - 1;
		...
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final long remainingBlocks = format.byteCount(packedIntsVersion, remaining, bitsPerValue);
		  long remainingBlocks = Format.byteCount(PackedIntsVersion, remaining, BitsPerValue_Renamed);
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final int blocksToRead = (int) Math.min(remainingBlocks, nextBlocks.length);
