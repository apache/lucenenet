...
namespace Lucene.Net.Search
{
    ...
    public class DisjunctionMaxQuery : Query, IEnumerable<Query>
    {
        ...
        {
            private readonly DisjunctionMaxQuery enclosingInstance;
            ...
            public DisjunctionMaxWeight(DisjunctionMaxQuery enclosingInstance, IndexSearcher searcher)
            {
            ...
            public override float ValueForNormalization
            {
                get
                {
                    ...
                    float boost = enclosingInstance.Boost;
                    return (((sum - max) * enclosingInstance.tieBreakerMultiplier * enclosingInstance.tieBreakerMultiplier) + max) * boost * boost;
                }
            }
            ...
            {
                topLevelBoost *= enclosingInstance.Boost;  // Incorporate our boost
            ...
            {
                if (enclosingInstance.disjuncts.Count == 1)
                    return weights[0].Explain(context, doc);

                ComplexExplanation result = new ComplexExplanation();
                float max = 0.0f, sum = 0.0f;
                result.Description = enclosingInstance.tieBreakerMultiplier == 0.0f ? "max of:" : "max plus " + enclosingInstance.tieBreakerMultiplier + " times others of:";
                foreach (Weight wt in weights)
                {
                ...
                result.Value = max + (sum - max) * enclosingInstance.tieBreakerMultiplier;
                return result;
            }
        ...
        public override int GetHashCode()
        {
            return BitConverter.ToInt32(BitConverter.GetBytes(Boost), 0) 
                + BitConverter.ToInt32(BitConverter.GetBytes(tieBreakerMultiplier), 0) 
