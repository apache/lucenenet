                        ...
                        if (oper < commitPercent)
                        {
                            if (numCommitting.IncrementAndGet() <= maxConcurrentCommits)
                            {
                                ...
                                lock (outerInstance)
                                {
                                    ...
                                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(newReader.RefCount > 0)) Debugging.ThrowAssert();
                                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(outerInstance.reader.RefCount > 0)) Debugging.ThrowAssert();
