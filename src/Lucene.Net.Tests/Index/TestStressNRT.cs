                        ...
                        if (oper < commitPercent)
                        {
                            if (numCommitting.IncrementAndGet() <= maxConcurrentCommits)
                            {
                                ...
                                lock (outerInstance)
                                {
                                    ...
                                    if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(newReader.RefCount > 0);
                                    if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(outerInstance.reader.RefCount > 0);
