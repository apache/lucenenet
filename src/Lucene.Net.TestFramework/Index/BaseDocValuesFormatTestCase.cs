        ...
        private void DoTestNumericsVsStoredFields(Int64Producer longs)
        {
            using (Directory dir = NewDirectory())
            {
                ...
                using (RandomIndexWriter writer = new RandomIndexWriter(Random, dir, conf))
                {
                    ...
                    if (Debugging.ShouldAssert(numDocs > 256)) Debugging.ThrowAssert();
                    for (int i = 0; i < numDocs; i++)
                    {
                ...
                using (DirectoryReader ir = DirectoryReader.Open(dir))
                {
                    foreach (AtomicReaderContext context in ir.Leaves)
                    {
                        ...
                        for (int i = 0; i < r.MaxDoc; i++)
                        {
                            ...
                            {
                                if (Debugging.ShouldAssert(docValues != null)) Debugging.ThrowAssert();
                                long ord = docValues.NextOrd();
                                if (Debugging.ShouldAssert(ord != SortedSetDocValues.NO_MORE_ORDS)) Debugging.ThrowAssert();
                                docValues.LookupOrd(ord, scratch);
                                Assert.AreEqual(stringValues[j], scratch.Utf8ToString());
                            }
                            if (Debugging.ShouldAssert(docValues == null || docValues.NextOrd() == SortedSetDocValues.NO_MORE_ORDS)) Debugging.ThrowAssert();
                        }
                    }
                } // ir.Dispose();
            } // dir.Dispose();
