            ...
            using (Directory directory = NewDirectory())
            {
                ...
                using (IndexReader ireader = DirectoryReader.Open(directory)) // read-only=true
                {
                    ...
                    for (int i = 0; i < hits.ScoreDocs.Length; i++)
                    {
                        ...
                        if (Debugging.AssertsEnabled && Debugging.ShouldAssert(ireader.Leaves.Count == 1)) Debugging.ThrowAssert();
        ...
        private void DoTestNumericsVsStoredFields(Int64Producer longs)
        {
            using (Directory dir = NewDirectory())
            {
                ...
                using (RandomIndexWriter writer = new RandomIndexWriter(Random, dir, conf))
                {
                    ...
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(numDocs > 256)) Debugging.ThrowAssert();
                    for (int i = 0; i < numDocs; i++)
                    {
                ...
                using (DirectoryReader ir = DirectoryReader.Open(dir))
                {
                    foreach (AtomicReaderContext context in ir.Leaves)
                    {
                        ...
                        for (int i = 0; i < r.MaxDoc; i++)
                        {
                            ...
                            {
                                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(docValues != null)) Debugging.ThrowAssert();
                                long ord = docValues.NextOrd();
                                if (Debugging.AssertsEnabled && Debugging.ShouldAssert(ord != SortedSetDocValues.NO_MORE_ORDS)) Debugging.ThrowAssert();
