...
#pragma warning restore xUnit1013
        {
            try
            {
                ...
                random = null; // Remove our random - random numbers after this point will be generated by LuceneTestFrameworkInitializer.random
            }
            catch (Exception ex)
            {
                // Write the stack trace so we have something to go on if an error occurs here.
        ...
        public static Random Random
        {
            ...
            {
#if TESTFRAMEWORK_NUNIT
                if (TestExecutionContext.CurrentContext.CurrentTest.IsSuite)
                {
                    // This value is set to null at the end of OneTimeTearDown to prevent polluting the next instance.
                    return random ?? (random = new Random(Randomizer.InitialSeed));
                }
