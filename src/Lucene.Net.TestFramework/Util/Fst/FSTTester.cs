    ...
    public class FSTTester<T>
    {
        ...
        private static BytesRef ToBytesRef(Int32sRef ir)
        {
            ...
            for (int i = 0; i < ir.Length; i++)
            {
                ...
                if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(x >= 0 && x <= 255);
                br.Bytes[i] = (byte)x;
            }
            br.Length = ir.Length;
            return br;
        ...
        {
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(prefixLength == null || prefixLength.Length == 1);
        ...
        private void VerifyPruned(int inputMode, FST<T> fst, int prune1, int prune2)
        {
            ...
            using (var it = prefixes.GetEnumerator())
            {
                while (it.MoveNext())
                {
                    ...
                    {
                        if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(prune2 > 0);
