...
#pragma warning disable 612, 618
    internal class Lucene40DocValuesWriter : DocValuesConsumer
    {
        ...
        private void AddVarStraightBytesField(FieldInfo field, IndexOutput data, IndexOutput index, IEnumerable<BytesRef> values)
        {
            ...
            int maxDoc = state.SegmentInfo.DocCount;
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(maxDoc != int.MaxValue); // unsupported by the 4.0 impl
            ...
            // write sentinel
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(currentPosition == maxAddress);
            w.Add(currentPosition);
            w.Finish();
        }
        ...
        {
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(i >= 0 && i <= short.MaxValue);
            if (i < 128)
            {
        ...
        private void AddVarSortedBytesField(FieldInfo field, IndexOutput data, IndexOutput index, IEnumerable<BytesRef> values, IEnumerable<long?> docToOrd)
        {
            ...
            if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(valueCount != int.MaxValue); // unsupported by the 4.0 impl
