...
#pragma warning disable 612, 618
    internal class Lucene40DocValuesWriter : DocValuesConsumer
    {
        ...
        private void AddVarStraightBytesField(FieldInfo field, IndexOutput data, IndexOutput index, IEnumerable<BytesRef> values)
        {
            ...
            int maxDoc = state.SegmentInfo.DocCount;
            if (Debugging.ShouldAssert(maxDoc != int.MaxValue)) Debugging.ThrowAssert(); // unsupported by the 4.0 impl
            ...
            // write sentinel
            if (Debugging.ShouldAssert(currentPosition == maxAddress)) Debugging.ThrowAssert();
            w.Add(currentPosition);
            w.Finish();
        }
        ...
        {
            if (Debugging.ShouldAssert(i >= 0 && i <= short.MaxValue)) Debugging.ThrowAssert();
            if (i < 128)
            {
        ...
        private void AddVarSortedBytesField(FieldInfo field, IndexOutput data, IndexOutput index, IEnumerable<BytesRef> values, IEnumerable<long?> docToOrd)
        {
            ...
            if (Debugging.ShouldAssert(valueCount != int.MaxValue)) Debugging.ThrowAssert(); // unsupported by the 4.0 impl
