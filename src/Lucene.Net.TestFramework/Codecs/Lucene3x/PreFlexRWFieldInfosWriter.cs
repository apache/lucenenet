        ...
        public override void Write(Directory directory, string segmentName, string segmentSuffix, FieldInfos infos, IOContext context)
        {
            ...
            try
            {
                ...
                foreach (FieldInfo fi in infos)
                {
                    ...
                    {
                        bits |= IS_INDEXED;
                        if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(fi.IndexOptions == IndexOptions.DOCS_AND_FREQS_AND_POSITIONS || !fi.HasPayloads);
                    ...
                    if (Debugging.AssertsEnabled) Debugging.ThrowAssertIf(fi.Attributes == null); // not used or supported
                }
                success = true;
            }
            finally
