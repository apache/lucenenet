        ...
        public override void Write(Directory directory, string segmentName, string segmentSuffix, FieldInfos infos, IOContext context)
        {
            ...
            try
            {
                ...
                foreach (FieldInfo fi in infos)
                {
                    ...
                    {
                        bits |= IS_INDEXED;
                        if (Debugging.AssertsEnabled && Debugging.ShouldAssert(fi.IndexOptions == IndexOptions.DOCS_AND_FREQS_AND_POSITIONS || !fi.HasPayloads)) Debugging.ThrowAssert();
                    ...
                    if (Debugging.AssertsEnabled && Debugging.ShouldAssert(fi.Attributes == null)) Debugging.ThrowAssert(); // not used or supported
                }
                success = true;
            }
            finally
