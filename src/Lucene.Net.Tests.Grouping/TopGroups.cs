...
namespace Org.Apache.Lucene.Search.Grouping
{
	...
	public class TopGroups<GROUP_VALUE_TYPE>
	{
		...
		public TopGroups(Org.Apache.Lucene.Search.Grouping.TopGroups<GROUP_VALUE_TYPE> oldTopGroups
			, int totalGroupCount)
		{
		...
		public static Org.Apache.Lucene.Search.Grouping.TopGroups<T> Merge<T>(Org.Apache.Lucene.Search.Grouping.TopGroups
			 ...
			 scoreMergeMode)
		{
			...
			foreach (Org.Apache.Lucene.Search.Grouping.TopGroups<T> shard in shardGroups)
			{
			...
			for (int groupIDX = 0; groupIDX < numGroups; groupIDX++)
			{
				...
				for (int shardIDX = 0; shardIDX < shardGroups.Length; shardIDX++)
				{
					...
					Org.Apache.Lucene.Search.Grouping.TopGroups<T> shard_1 = shardGroups[shardIDX];
			...
			if (totalGroupCount != null)
			{
				Org.Apache.Lucene.Search.Grouping.TopGroups<T> result = new Org.Apache.Lucene.Search.Grouping.TopGroups
					<T>(groupSort.GetSort(), docSort == null ? null : docSort.GetSort(), totalHitCount
					, totalGroupedHitCount, mergedGroupDocs, totalMaxScore);
				return new Org.Apache.Lucene.Search.Grouping.TopGroups<T>(result, totalGroupCount
					);
			}
			else
			{
				return new Org.Apache.Lucene.Search.Grouping.TopGroups<T>(groupSort.GetSort(), docSort
